...
| **Vindex sphere: four counts** (Aktivni predmeti / Aktivni rokovi / Novi klijenti / Nepročitani dokumenti) | New for this sprint — no existing endpoint computes these four exact numbers (see UX Architecture §9) | An instant, wordless read of "how loud is today" before reading a single line of text | Every login | Ambient | **Black header band, above the paper work surface** | Always visible, values are representative/dummy in this prototype | Directly requested by the brief (§8) as the functional brand object; placed where it cannot compete with or duplicate Traži pažnju/Uskoro — it summarizes, it does not replace, the detailed streams below it |
...
| Dokument intake / "Dodaj spis" | `predmeti` / `dosije` upload flows | Add evidence to a case | Frequent, but always in service of a specific case | High when needed | **Not on Home — contextual, inside a Predmet** | Contextual only | A document upload with no case attached is not a real action in this product (a spis belongs to a predmet); promoting it to Home would invent a capability that doesn't exist. Rejected. |
...
## Final Home shape (result of the table above)
...
1. **Black header band**: wordmark context (inherited from shell), the sphere with its four
   ambient counts, one summary sentence (existing `sazetak` mechanism), primary "Novi predmet"
   action.
...
folded into existing mechanisms instead of becoming new blocks**, and **only the sphere is genuinely
new** — everything else on the final Home surface was already there before this sprint began.
