...
def test_upload_rejects_scanned_pdf():
    with patch("uploaded_doc.extractor.extract", return_value=("", True, False, None, None)):
        with open(UGOVOR_PDF, "rb") as f:
            resp = client.post(
                "/api/dokument/upload",
...
def test_upload_rejects_empty_chunks():
    ...
    with patch("uploaded_doc.extractor.extract", return_value=("some text", False, False, None, None)), \
