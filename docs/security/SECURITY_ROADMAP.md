...
| SEC-003 | ~~Wrap all GPT call sites in existing `prompt_guard.wrap_for_ai()`; extend `analyze()` blocking to document-ingestion paths~~ **DONE 2026-07-23** — central guard patched onto the OpenAI SDK's `Completions.create`/`AsyncCompletions.create` at the class level, all 130 call sites (53 files) structurally protected with zero per-call-site edits; 12 regression tests; full suite 1737 passed | Very High | Closed the largest AI-safety gap | Low–Medium |
| SEC-002 | ~~Founder decision + fix on GDPR erasure scope~~ **MESSAGE FIX DONE 2026-07-23** — founder decided message-only fix for now (`routers/gdpr.py`), real anonymization/deletion deferred pending a formal Advokatska Data Retention Politika; regression-guarded by `tests/test_gdpr_delete.py` | Very High | Removed a materially false compliance claim; legal exposure | Small (message only) — data-handling decision deliberately still open |
