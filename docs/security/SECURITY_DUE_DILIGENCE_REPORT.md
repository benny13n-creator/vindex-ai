...
## Score Update 9 — 2026-07-24, SEC-002 Data Retention & GDPR/ZZPL Cleanup — final finding of this engagement

SEC-002 had been open since the original audit in two parts: a message-accuracy fix (closed 2026-07-23) and the underlying question this document has cited as the #1 item on the path to 90+ in every score update since Score Update 3 — "a founder-approved formal Data Retention Policy." This update closes that.

A read-only analysis phase found that this codebase's entire retention story was aspirational, not real: `migrations/043_security_bulletproof.sql` and `044_anomaly_detection.sql` both contain "Auto-cleanup: briše zapise starije od 90 dana (pokrenuti kao cron job)" followed by a `DELETE` statement that was commented out and never wired to anything; the one genuinely working piece (a 24-hour TTL on Pinecone's temporary document-upload buffers) only cleaned up opportunistically, piggybacked on unrelated new uploads, never on a reliable schedule. New `services/retention_service.py::execute_retention_cleanup()` finally executes what those comments always intended: `security_events`/`user_daily_activity` at 90 days, `ai_forensics` at 180 days (kept longer, as forensic evidence for AI-answer disputes), plus the Pinecone buffer cleanup now called reliably instead of by chance. `predmeti`/`klijenti`/`dokumenti` remain deliberately, permanently untouched — the same statutory-retention decision (Zakon o advokaturi) already reflected in SEC-002's Part A message fix — and `audit_immutable` is architecturally exempt (its own DB trigger blocks all DELETE/UPDATE by design, for tamper-evidence).

**Implementing this surfaced a significant, unrelated finding, the same class this project has now found three times (SEC-034 in migrations, SEC-005 in the rate limiter, now this): a control that looks live but silently isn't.** `POST /api/cron/daily` was registered twice — once in `routers/email_notif.py` (email reminders only) and once in `api.py` (a considerably richer "unified" dispatcher with an idempotency guard, heartbeat, workflow escalations, weekly `zakon_monitoring`, and memory cleanup). Starlette dispatches every request to the first matching route registered; direct inspection of `app.routes` confirmed `email_notif`'s simpler handler was registered first, meaning **`api.py`'s entire richer dispatcher — five built modules — had never executed once since it was written.** Fixed: the duplicate route was removed from `email_notif.py`; its three underlying functions are now called directly from `api.py`'s dispatcher as new modules alongside the retention cleanup, reviving all five previously-dead modules as a side effect of this fix, not just adding a sixth.

A second self-caught correction, in the same spirit as this project's established practice of verifying rather than assuming: the read-only analysis phase had initially claimed `usage_events` and `response_audit` were "confirmed dead" tables (the same class as SEC-034/SEC-035). That claim was wrong — both are live, actively-used tables (including, for `usage_events`, as part of the GDPR data export itself) — caught immediately when the retention module's own tests failed on first run, and corrected before it became a permanent, wrong claim in this document.

| Category | Prior | Updated | What changed |
|---|---|---|---|
| Compliance Readiness | 66 | 72 | The single most-repeated blocker in every prior score update — "no formal Data Retention Policy exists" — is now a real, executing, tested cleanup path for the tables where a retention period could be reasonably decided without a legal consultation. The largest movement in this update because it directly resolves the most persistently-cited gap |
| Privacy | 82 | 84 | A genuine data-minimization improvement (security/audit logs no longer accumulate indefinitely) — smaller movement than Compliance Readiness because Privacy's largest driver (SEC-031) was already resolved in an earlier update |
| Infrastructure | 63 | 65 | The cron-collision fix stands on its own merit here, independent of retention specifically — a second confirmed instance this cycle of "a maintenance control that looked live but wasn't," now fixed, reviving 5 previously-dead modules |
| All other categories | — | unchanged | Not touched this update |

**Updated weighted overall: ~70/100.**

**Why not a bigger jump, given this closes the most-cited remaining item**: the same standing rule applies one more time — code-verified and test-verified (21 new tests, including tests that directly inspect `app.routes` to prove the collision is actually resolved, not just that the code reads correctly), not production-verified. Whether the external cron scheduler's next real invocation actually shows all 9 modules executing and rows actually being deleted in production is a live check only the founder can run, the same honest gap carried by every other SEC-0xx item closed this way in this engagement. `usage_events`/`response_audit` remain open, not as "possibly dead" anymore (that was wrong) but as "live tables with no retention period decided yet" — a real, smaller, future scoping question, correctly re-labeled rather than silently dropped.
