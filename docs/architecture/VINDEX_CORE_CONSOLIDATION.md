...
- **Current state — no single owner:** Genome's `rokovi_kriticni` field (extracted on document upload) and the case pipeline's own deadline-extraction step (on case creation) are two independent extraction paths that never reference each other or write to a shared table.
- **Target:** a single Deadline Engine — this does not exist yet as a distinct module and needs to be designed, not just repointed. Until it exists, Genome's `rokovi_kriticni` is the interim owner (it is closer to the "single structured record" shape); the case pipeline's extraction step should stop computing its own version and instead read from Genome once the write-path in §1.3 exists.
