...
1.5. **Phase 0.5 — Reality Calibration (added 2026-07-23, gates Phase 1).** The question stops being *"does it run"* and becomes *"is it actually better than what it's meant to replace."* Not implementation work — an empirical evaluation, same discipline this project has used every time before merging or replacing a system (G-027/G-030/G-034 precedent: measure before architecting, never assume).
   ...
   **Evaluation framework — built, not a one-off script (founder, 2026-07-23): `evaluation/phase_0_5/`** (`metrics.py`, `run.py`, `compare.py`, `report.py`, `datasets/`, `outputs/`) — reusable for future evaluations (LRE v2, Precedent Engine, Draft Engine, Adversarial Review). Full process in `evaluation/phase_0_5/README.md`. Key design points, all founder-specified:
   ...
   - **Hardened same day (2026-07-23), founder second-pass review:** blind-label assignment now uses true randomness (`SystemRandom`), not a `predmet_id`-derived seed — deriving the A/B split from the case id was itself a guessable pattern across many cases. Dataset manifest extended with `complexity`/`ground_truth_available`/`reviewer` fields and a `selection_method` field that `run.py` checks and warns on if it isn't `"prvih_30_novih_po_kategoriji"` — encodes the founder's explicit anti-selection-bias rule ("ako uzmeš prvih 30 'najzanimljivijih' predmeta, dobijaš iskrivljenu sliku — mora biti prvih 30 NOVIH po kategoriji"). `compare.py`/`report.py` now produce a **per-profile breakdown matrix**, not just an overall average — a tie overall can hide LRE winning big in one case type and losing big in another. Two new files: `FAILURE_LOG.md` (records *why* a case was lost, not a bug list — "zlato za razvoj") and `PHASE_0_5_DECISION.md` (GO / GO WITH FIXES / NO GO, criteria fixed before any data exists, same discipline as `G030_NEXT_ACTION_DECISION_MODEL.md`). Full suite: 1719 passed.
