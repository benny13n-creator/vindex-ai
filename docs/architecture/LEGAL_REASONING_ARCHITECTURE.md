...
1.5. **Phase 0.5 — Reality Calibration (added 2026-07-23, gates Phase 1).** The question stops being *"does it run"* and becomes *"is it actually better than what it's meant to replace."* Not implementation work — an empirical evaluation, same discipline this project has used every time before merging or replacing a system (G-027/G-030/G-034 precedent: measure before architecting, never assume).
   ...
   **Hard requirement before this phase can produce a trustworthy result, not optional polish:** `retrieval_agreement`'s current substring-match implementation (§Phase 0 note above) must be replaced with an identity-based chain — `Reasoning Node → Evidence ID → Legal Source ID → Retrieved Citation ID` — before Phase 0.5 numbers are treated as meaningful. Founder's own reasoning, kept verbatim because it's the right bar: *"Jednog dana će advokat pitati 'zašto je confidence pao?' — a odgovor 'substring nije pronađen' nije pravno objašnjiv."* Substring matching may still run during Phase 0.5 data collection, but any confidence number it produces must be labeled as provisional in the comparison, not presented as final.
