...
|---|---|---|---|---|
| 29 | `POST /v1/webhook/clio` | `routers/integracije.py:275-314` | `vindex_user_id` taken directly from the attacker-controlled webhook body; only gated by a platform-wide shared HMAC secret, not a per-user credential | `LAMBDA-OWN-001` |
| 30 | `/api/dokument/pitanje`, `/analiza`, `/rokovi`, `/klasifikuj-sesija` | `routers/dokument.py` (full file) | Pinecone `session_id` has no `user_id` binding anywhere — any caller who obtains a valid session_id (unguessable 128-bit UUID, but no secondary check if ever leaked) gets full read access to that document's content/analysis/deadlines | Pre-existing `SEC-039` (High), re-confirmed not re-opened |
