      ...
      if (bodyEl) {
        ...
        if (_stratAktivniModul === 'court_predictor') {
          bodyEl.innerHTML += '<div id="strat-battle-report-wrap" style="margin-top:1rem;">'
            ...
            + '</div>'
            + '<div id="strat-predictor-argrep-wrap" style="margin-top:.4rem;">'
            + '<textarea id="strat-argrep-lista" placeholder="Argumenti koje planirate da koristite — po jedan u redu" rows="3" style="width:100%;padding:.4rem .55rem;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:2px;color:#fff;font-size:.74rem;box-sizing:border-box;resize:vertical;margin-bottom:.35rem;"></textarea>'
            + '<button class="vx-btn vx-btn-secondary" onclick="stratArgumentReputation()" style="width:100%;">Proveri reputaciju argumenata (2 kredita)</button>'
            + '</div>'
...
async function stratArgumentReputation() {
  var listaEl = document.getElementById('strat-argrep-lista');
  var tipEl   = document.getElementById('strat-tip-postupka');
  var wrapEl  = document.getElementById('strat-argrep-result-wrap');
  if (!listaEl || !wrapEl) return;
  if (!currentUserIsPro) { wrapEl.innerHTML = '<div class="strat-pro-gate">Dostupno samo PRO korisnicima.</div>'; return; }
  var argumenti = listaEl.value.split('\n').map(function(s){ return s.trim(); }).filter(Boolean);
  if (!argumenti.length) { wrapEl.innerHTML = '<div class="strat-error">Unesite bar jedan argument (po jedan u redu).</div>'; return; }

  wrapEl.innerHTML = '<div class="strat-loading">⏳ Proveravam reputaciju argumenata...</div>';
  try {
    var res = await fetch(BASE_URL + '/api/predictor/argument-reputation', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + (currentSession ? currentSession.access_token : '') },
      body: JSON.stringify({ tip_spora: _STRAT_TIP_TO_SPORA[tipEl ? tipEl.value : 'gradjansko'] || 'parnicno', argumenti: argumenti })
    });
    if (res.status === 403) { wrapEl.innerHTML = '<div class="strat-pro-gate">Dostupno samo PRO korisnicima.</div>'; return; }
    if (!res.ok) throw new Error('Server greška: ' + res.status);
    var d = await res.json();
    var bojaHex = { zelena: '#4ade80', 'žuta': '#facc15', zuta: '#facc15', crvena: '#f87171' };
...
function renderKompletnaAnaliza(data) {
  ...
  html += '<div style="background:linear-gradient(135deg,rgba(99,102,241,.15),rgba(255,255,255,0.05));border:1px solid rgba(99,102,241,.35);border-radius:3px;padding:1rem 1.1rem;margin-bottom:.9rem;">';
  ...
  if (stavInfo) html += '<span style="padding:.2rem .7rem;border-radius:2px;font-size:.72rem;font-weight:800;letter-spacing:.06em;background:'+stavInfo[0]+';color:#fff;">'+_htmlEsc(stavInfo[1])+'</span>';
  html += '</div>';
  ...
  if (sinteza.detektovani_konflikti && sinteza.detektovani_konflikti.length) {
    html += '<div style="margin:.5rem 0;padding:.5rem .6rem;background:rgba(234,179,8,.07);border:1px solid rgba(234,179,8,.2);border-radius:2px;">';
  ...
  if (sinteza.sistemsko_upozorenje) {
    html += '<div style="margin:.5rem 0;padding:.5rem .6rem;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);border-radius:2px;font-size:.78rem;color:rgba(255,200,200,.9);">'+_htmlEsc(sinteza.sistemsko_upozorenje)+'</div>';
  }
  ...
  koraciDef.forEach(function(kd) {
    ...
    html += '<details style="margin:.3rem 0;border:1px solid rgba(255,255,255,.1);border-radius:3px;overflow:hidden;">';
...
function _rka_korakGeneric(k) {
  ...
  var html = '';
  if (k.summary) {
    html += '<div style="font-size:.82rem;line-height:1.5;color:rgba(230,225,255,.8);margin-bottom:.5rem;padding:.4rem .5rem;background:rgba(99,102,241,.07);border-radius:2px;">'+_htmlEsc(k.summary)+'</div>';
  }
  function _rka_field(key, val) {
    ...
    if (Array.isArray(val)) {
      ...
      html += '<pre style="font-size:.73rem;color:rgba(220,215,255,.7);background:rgba(255,255,255,.03);padding:.4rem;border-radius:2px;white-space:pre-wrap;margin:.2rem 0;">'+_htmlEsc(JSON.stringify(val,null,2))+'</pre>';
    } else {
...
function _rka_korak5(k) {
  ...
  if (presuda.izreka) {
    ...
    html += '<div style="text-align:center;padding:.65rem;background:'+vBg+';border:1px solid '+vBrd+';border-radius:2px;margin-bottom:.6rem;">';
...
function _extractBrzaProcena(rawText) {
  ...
  endMarkers.forEach(function(m) {
    var idx = rawText.indexOf(m, afterStart);
...
/* Renderuje "--- SUDSKA PRAKSA" blok (numerisane stavke) kao kartice umesto plain teksta. */
function _renderSudskaPraksaBlock(val) {
  var items = val.split(/\n(?=\d+\.\s)/).map(function(s){ return s.trim(); }).filter(Boolean);
  if (!items.length) return '<div class="resp-section-body">' + escHtml(val).replace(/\n/g,'<br>') + '</div>';
...
function formatResponse(rawText) {
  ...
  if (!isStructured) return _heroHtml + '<div class="resp-plain">'+escHtml(rawText)+'</div>';
  ...
  found.forEach(function(s,i){
    ...
    } else if (s.cls === 'resp-sudska-praksa') {
      bodyHtml = _renderSudskaPraksaBlock(val);
...
function _predInlineEdit(spanId, field, inputType) {
  ...
  inp.style.cssText = 'width:100%;max-width:150px;padding:2px 5px;background:' + (isSelect ? '#131929' : 'rgba(255,255,255,0.12)') + ';border:1px solid rgba(0,212,255,0.35);border-radius:2px;color:#e2eeff;font-size:0.78rem;outline:none;box-sizing:border-box;';
...
function _cioRender(iz, meta) {
  ...
  var html = '<div style="background:linear-gradient(135deg,rgba(255,255,255,0.02),rgba(124,58,237,0.04));border:1px solid rgba(0,212,255,0.09);border-radius:3px;overflow:hidden;">';
...
function _caseDnaRender(dna, predmetId) {
  ...
  if (ned.length) {
    html += '<div style="margin-bottom:0.5rem;background:rgba(251,191,36,0.05);border:1px solid rgba(251,191,36,0.2);border-radius:2px;padding:0.35rem 0.5rem;">';
...
function _genomHeatmapDrill(predmetId, dimension) {
  ...
  var content = '<div style="margin:3px 0 6px;padding:6px 8px;background:rgba(0,0,0,0.28);border-radius:2px;border-left:2px solid rgba(255,255,255,0.09);">';
...
function _ccc_render(el, d) {
  ...
  el.innerHTML = [
    ...
    '<div style="display:flex;align-items:center;gap:.8rem;margin-bottom:.75rem;padding:.75rem 1rem;',
      ...
      'border:1px solid rgba(0,212,255,0.12);border-radius:3px;">',
      '<div style="flex:1;min-width:0;">',
  ...
  deduped.forEach(function(x,i) {
    ...
    if (x.s.isZalba) {
      ...
      if (ocena) {
        ...
        badgeHtml = '<div style="display:inline-flex;align-items:baseline;gap:0.45rem;background:'+bg+';border:1px solid '+bc+';border-radius:2px;padding:0.3rem 0.9rem;margin-bottom:0.55rem;">'
...
async function _outcome_feedback_show(predmetId) {
  ...
  try {
    ...
    if (d.win_rate !== null && d.win_rate !== undefined) {
      ...
      winBadge = '<span style="background:rgba(0,0,0,0.3);border:1px solid ' + wc + ';color:' + wc + ';border-radius:2px;padding:.15rem .5rem;font-size:.72rem;font-weight:700;margin-left:.5rem;">Win rate ' + d.win_rate + '%</span>';
    }
