...
_PARNICA_TRIGGERS = [
    ...
    # KZ criminal terms routed to PARNICA (criminal litigation)
    "kazna za", "kradja", "razbojnistvo", "pronevera", "prevara",
    "silovanje", "nasilje u porodici", "opojne droge", "narkotik",
    "nuzna odbrana", "uslovn", "ubistvo", "ubojstvo",
]
...
def klasifikuj_pitanje(query: str) -> str:
    ...
    Prioritet: KZ_OVERRIDE > COMPLIANCE > PORESKI > PARNICA > DEFINICIJA.
    ...
    for term in _KZ_OVERRIDE_TRIGGERS:
        if _normalizuj(term) in q:
            logging.info("Klasifikacija: '%s' → PARNICA/KZ (kz_override: %s)", query[:50], term)
            return "PARNICA"
