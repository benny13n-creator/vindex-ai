...
| ATLAS-004 | Unify or explicitly separate Genome's `_emit_genome_event` correlation_id from this mission's per-AI-call correlation_id | 4 | Architecture decision | Small (once decided) | NEEDS_SCOPING | Two independently-generated correlation concepts currently coexist — may be correct as-is (different questions) or worth unifying. |
...
| ATLAS-006 | Populate `audit_reference`, cross-link to `audit_immutable` | 6 | SENT-004 (`AUDITABLE_ACTIONS` widening) | Medium | NEEDS_SCOPING — founder decision | Audit Link Coverage is the weakest of this mission's 4 metrics (~5-10%) specifically because this and SENT-004 are still open. |
