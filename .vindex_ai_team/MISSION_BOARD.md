...
| SENT-004 | Widen `AUDITABLE_ACTIONS` + wire Strategy Engine/Copilot/Briefing/Case Pipeline/Task Engine into durable audit | 4 | Founder decision on which actions warrant durable hash-chained provenance | Medium-Large | **PARTIALLY DONE (2026-08-03, Mission Ledger)** | Strategy Engine (all 9 endpoints), Copilot's case-analysis handler, Task Engine, and Briefing now wired (`strategija_generisana`/`copilot_analiza_predmeta`/`zadaci_ai_analiza_complete`/`briefing_generisan`, all correlation-linked). Case Pipeline's own 9 internal steps and Copilot's other ~9 handlers remain unwired — see `LEDGER-004`. |
...
| ATLAS-005 | Extend explicit `case_context()` wiring to the ~45 AI call sites not covered this mission | 5 | none (mechanical) | Large (many small edits) | TODO | Model/prompt/output/correlation_id already captured for these automatically; only case-linking (`predmet_id`/`document_id`) is missing, with `module_name` falling back to stack-introspection instead of an explicit tag. |
| ATLAS-006 | Populate `audit_reference`, cross-link to `audit_immutable` | 6 | SENT-004 (`AUDITABLE_ACTIONS` widening) | Medium | **PARTIALLY DONE (2026-08-03, Mission Ledger)** | `audit_reference` now defaults to `correlation_id` (the actual join key). `AUDITABLE_ACTIONS` widened + `log_action` wired for 5 representative modules (Genome, Strategy Engine, Copilot's case-analysis, Task Engine, Briefing) — see `SENT-004` below for the remaining ~15 features. |
...
| LEDGER-004 | Extend `AUDITABLE_ACTIONS`/`log_action` wiring to the remaining ~15 AI features (Court Predictor, Drafting, document classification, Copilot's other ~9 handlers, etc.) | 4 | none (mechanical, many files) | Large | TODO | Same proven pattern as the 5 modules this mission wired — Audit Link Coverage is currently ~25%, target ≥95%. |
